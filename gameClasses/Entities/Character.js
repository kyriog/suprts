...
var Character = IgeEntity.extend(
{
	...
	init: function () 
	{
		...
		self.xCurrentChunk = 0;
		self.yCurrentChunk = 0;
	...
	walkTo: function (x, y) 
	{
		self.log('Character Walking to (' + x + ',' + y + ')');
		console.log('Character Walking to (' + x + ',' + y + ')');
		var self = this,
		distX = x - this.translate().x(),
		distY = y - this.translate().y(),
		distance = Math.distance(this.translate().x(),this.translate().y(),x,y),speed = 0.1,time = (distance / speed);
	...
	tick: function (ctx) 
	{
		...
		if(ige.isServer)
		{
			var c = this.worldPosition().thisTo2d();
			var coord = {x: Math.round(c.x/40), y: Math.round(c.y/40)};
			
			var xChunk = coord.x - coord.x%10;
			var yChunk = coord.y - coord.y%10;
			
			if(coord.x < 0)
			{
				xChunk = ( Math.ceil(coord.x / 10) * 10);
				if(coord.x % 10 != 0)
				{
					xChunk = xChunk - 10;
				}
			...
			if(coord.y < 0)
			{
				yChunk = ( Math.ceil(coord.y / 10) * 10);
				if(coord.y % 10 != 0)
				{
					yChunk = yChunk - 10;
				}
